"""
Train LSTM forecasting model on ETTm1 dataset.

Usage:
    python scripts/train_lstm.py --device mps --epochs 30
"""
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        # Validate
        val_loss, val_metrics = validate(model, val_loader, criterion, device, tracker=val_tracker)
        ...
        if val_metrics['mae'] < best_val_mae:
            ...
            checkpoint_path = checkpoint_dir / f'lstm_{dataset_name}_best.pt'
            torch.save({
                'epoch': epoch,
    ...
    # Load best model
    print(f"\nLoading best model")
    checkpoint = torch.load(checkpoint_dir / f'lstm_{dataset_name}_best.pt')
    ...
    results_path = Path('results') / f'lstm_{dataset_name}_training_results.pkl'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train LSTM forecasting model")
