"""
Train Mamba forecasting model on ETTm1 dataset.

Usage:
    rye run python scripts/train_mamba.py --device cuda
    rye run python scripts/train_mamba.py --device cuda --epochs 50 --d_model 256
"""
...
def main(args):
    print("=" * 80)
    print("Training Mamba Forecasting Model")
    print("=" * 80)
    ...
    # Create model
    print(f"\nCreating Mamba forecasting model")
    ...
    print(f"  Number of patches: {context_length // args.patch_len}")
    ...
    for epoch in range(args.epochs):
        ...
        if val_metrics['mae'] < best_val_mae:
            ...
            checkpoint_path = checkpoint_dir / f'mamba_{dataset_name}_best.pt'
            torch.save({
                'epoch': epoch,
    ...
    # Load best model
    print(f"\nLoading best model")
    checkpoint = torch.load(checkpoint_dir / f'mamba_{dataset_name}_best.pt')
    ...
    results_path = Path('results') / f'mamba_{dataset_name}_training_results.pkl'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Mamba forecasting model")
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help='Compressed dimension for prediction head'
    )
